...
export const metadata: Metadata = publicMetadata({ title: `Pricing | Adaptive Fitness from ${membership.monthly.label}/mo`, description: `Keep training and nutrition aligned when schedules, travel, equipment, and progress change after a ${membership.trialDays}-day trial.`, path: "/pricing" });
const benefits = ["A training plan built around your schedule and equipment", "Short-on-time workouts that preserve the priority work", "Travel and equipment substitutions without starting over", "Nutrition targets explained in plain language", "Weekly progress and recovery decisions", "Cancel or manage billing from your account"];

export default function Pricing() { return <main className="pricing-page">
  <header className="pricing-hero chrome-shell"><p className="kicker">Your fitness. Handled.</p><h1>A fitness plan that changes when your life does.</h1><p>Your complete {membership.trialDays}-day trial starts when you create your account. STHENO keeps training and nutrition aligned as schedules, equipment, travel, and progress change.</p></header>
  <p className="pricing-value-anchor chrome-shell">You do the work. STHENO handles the plan—for less than the cost of a typical personal training session.</p>
  ...
  <section className="pricing-benefits chrome-shell"><div><p className="kicker">Included</p><h2>Your week changed. Your plan can too.</h2></div><ul>{benefits.map(x=><li key={x}><span>✓</span>{x}</li>)}</ul></section>
