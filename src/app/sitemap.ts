...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPaths = ["", "/assessment", "/pricing", "/contact", "/methodology", "/about", "/compare", "/library", "/tools", "/exercises", "/insights", ...pillars.filter((pillar)=>!["tools","exercises"].includes(pillar.slug)).map((pillar)=>`/library/${pillar.slug}`), ...tools.map((tool)=>`/tools/${tool.slug}`), ...articles.map((article)=>`/insights/${article.slug}`)];
