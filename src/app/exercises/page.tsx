...
export const metadata:Metadata={title:"Reviewed Exercise Library",description:"Browse hundreds of production-ready exercises with videos, setup, coaching cues, and muscle-matched alternatives.",alternates:{canonical:"/exercises"}};
export default async function Exercises(){
  const db=await createSupabaseServerClient();
  const{data}=db?await db.from("exercises").select("slug,name,purpose,primary_muscles,required_equipment,settings").eq("status","production").eq("review_status","reviewed").eq("public_indexable",true).eq("technical_review_status","reviewed").eq("editorial_review_status","reviewed").eq("visual_review_status","reviewed").eq("production_ready",true).order("name").limit(1000):{data:[]};
