...
const splits: Record<
  number,
  { name: string; focus: string; patterns: MovementPattern[] }[]
> = {
  2: [
    {
      name: "Full Body A",
  ...
  3: [
    {
      name: "Full Body A",
      focus: "full_body",
      patterns: ["squat", "horizontal_push", "horizontal_pull", "isolation"],
    },
    {
      name: "Full Body B",
      focus: "full_body",
      patterns: ["hinge", "vertical_push", "vertical_pull", "lunge"],
    },
    {
      name: "Full Body C",
      focus: "full_body",
  ...
  5: [
    ...
    },
    {
      ...
      focus: "lower",
      patterns: ["lunge", "hinge", "squat", "isolation"],
    },
  ],
};
function roleFor(index: number): ExerciseRole {
  return index === 0 ? "primary" : index < 3 ? "secondary" : "accessory";
}
function makePrescription(
  ...
  return {
    ...
    role,
    sets: methodology.sets[profile.experience][role],
    repMin: reps[0],
    repMax: reps[1],
    rir: methodology.rir[profile.experience],
    restSeconds: methodology.rest[role],
    estimatedMinutes: methodology.session.exerciseMinutes[role],
...
export function generateProgram(profile: TrainingProfile): TrainingProgram {
  if (
    ...
    throw new Error("SAFETY_CLEARANCE_REQUIRED");
  if (profile.daysPerWeek < 2 || profile.daysPerWeek > 5)
  ...
  const workouts: Workout[] = template.map((day, dayIndex) => {
    ...
    for (const pattern of day.patterns) {
      const role = roleFor(exercises.length);
      const minutes = exercises.reduce(
        (sum, e) => sum + e.estimatedMinutes,
        methodology.session.bufferMinutes,
      );
      if (
        minutes + methodology.session.exerciseMinutes[role] >
      ...
      try {
        ...
        if (!String(error).includes("NO_ELIGIBLE_EXERCISE")) throw error;
      }
    }
    if (exercises.length < 3)
      throw new Error(`INSUFFICIENT_EXERCISE_COVERAGE:${day.name}`);
    return {
      key: `day-${dayIndex + 1}`,
      name: day.name,
      ...
      exercises,
      reasonCodes: [
        `FREQUENCY_${profile.daysPerWeek}`,
        `FOCUS_${day.focus.toUpperCase()}`,
        profile.cardioDays && profile.cardioDays >= 3
          ? "CONCURRENT_CARDIO_MANAGED"
          : "SCHEDULE_HONORED",
      ],
    };
  });
  ...
  const id = createHash("sha256")
    .update(`${TRAINING_RULESET_VERSION}:${input}`)
    .digest("hex")
    .slice(0, 24);
  return {
    id,
    ...
    name: `${profile.daysPerWeek}-Day ${profile.goal.replaceAll("_", " ")} Foundation`,
    weeks: methodology.weeks,
    workouts,
    progressionPolicy: "double_progression",
    engineVersion: TRAINING_ENGINE_VERSION,
    rulesetVersion: TRAINING_RULESET_VERSION,
    ...
    reasonCodes: [
      ...
      "SCHEDULE_HONORED",
      "EQUIPMENT_CONFIRMED",
      "DETERMINISTIC_PRESCRIPTION",
    ],
    block: {
      blockIndex: 1,
      name: "Foundation",
      focus: profile.goal.replaceAll("_", " "),
      durationWeeks: methodology.weeks,
      currentWeek: 1,
      phase: "foundation",
      status: "active",
      reasonCodes: ["INITIAL_BLOCK_FROM_ASSESSMENT"],
    },
  };
}
